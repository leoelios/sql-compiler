...
class ReservedWord {
  ...
  static isReservedWord(token) {
    return Object.values(ReservedWord).includes(token.value);
  }
}
...
export function lexicalAnalysis(tokens) {
  ...
  return tokens.map((token) => {
    ...
    return {
      type: tokenTypes[token.toUpperCase()] || Type.UNKOWN,
      value: token,
    };
  });
}
...
function syntaticAnalysis(tokens) {
  ...
  const walk = (tokens, index) => {
    ...
    if (token.type === Type.UNKOWN) {
      if (
        lastToken.type === Type.UNKOWN &&
